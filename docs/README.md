...
## 2.5. Load Database Schema (if needed)
...
The GraphQL schema is defined in `backend/graphql/schema.py` and can be integrated with your FastAPI app.

### Option 1: Add to existing FastAPI app

In `backend/main.py`, uncomment and add these lines at the end (before `if __name__ == "__main__"`):
